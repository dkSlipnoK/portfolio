<header id="header" class="fixed-top">
    <div class="container d-flex align-items-center justify-content-between">
      <h1 class="logo"><a href="index.php">Portfolio</a></h1>
      <nav id="navbar" class="navbar">
        <ul>
          <li><a class="nav-link" href="index.php#hero">Accueil</a></li>
          <li><a class="nav-link" href="index.php#about">A propos</a></li>
          <li><a class="nav-link" href="index.php#bts">BTS SIO</a></li>
          <li class="dropdown"><a class="nav-link" href="index.php#services">Veille Techno</a></li>
          <li class="dropdown"><a href="index.php#work"><span>Projets</span> <i class="bi bi-chevron-down"></i></a>
            <ul>
                <li><a href="deploy.php" >Site Internet Chtac</a></li>
                <li class="dropdown"><a href="ap.php" ><span>Ateliers de Profesionnalisation</span><i class="bi bi-chevron-left"></i></a>
                    <ul>
                        <li><a href="ap.php#first" >Première année</a> </li>
                        <li><a href="ap.php#second" >Deuxième année</a> </li>
                    </ul>
                </li>
                <li class="dropdown"><a href="stage.php" ><span>Stages</span><i class="bi bi-chevron-left"></i></a>
                    <ul>
                        <li><a href="stage.php#first">Première année</a> </li>
                        <li><a href="stage.php#secondY">Deuxième année</a> </li>
                    </ul>
                </li>
                <li class="dropdown"><a href="sp.php"><span>Situations Professionnelles</span><i class="bi bi-chevron-left"></i> </a>
                    <ul>
                        <li><a href="sp.php#first"><span>Situation 1</span></a></li>
                        <li><a href="sp.php#second"><span>Situation 2</span></a> </li>
                    </ul>
                </li>
            </ul>
          </li>
            <li class="dropdown"><a href="index.php#profil"> <span>Profil</span> <i class="bi bi-chevron-down"></i></a>
            <ul>
              <li><a href="CV.php" >CV</a></li>
              <li><a href="certif.php" >Certifications</a></li>
            </ul>
          </li>
        </ul>
        <i class="bi bi-list mobile-nav-toggle"></i>
      </nav>
    </div>
  </header>
